...
def benchmark_query_performance(
    ...
    try:
        ...
        logger.info(f"Running benchmark with {len(test_queries)} queries, {num_runs} runs each")
        ...
        for query in test_queries:
            ...
            for run in range(num_runs):
                ...
                if response.objects:
                    scores = [obj.metadata.score for obj in response.objects if obj.metadata.score]
                    if scores:
                        score_variance = statistics.variance(scores) if len(scores) > 1 else 0
                        accuracy_scores.append(max(scores) - score_variance)  # Higher = better
        ...
        metrics = {
            ...
            "query_time_std": round(statistics.stdev(query_times) if len(query_times) > 1 else 0, 2),
            "avg_accuracy_score": round(statistics.mean(accuracy_scores) if accuracy_scores else 0, 4),
        ...
        for ef_value in ef_values_to_test:
            ...
            try:
                ...
                metrics["simulated_ef_impact"] = {
                    ...
                    "accuracy_improvement": min(ef_multiplier * 0.1, 0.3)  # Diminishing returns
                }
...
def print_benchmark_report(results: Dict[str, Any]):
    ...
    for ef_config, metrics in ef_benchmarks.items():
        if "error" not in metrics:
            ...
            print(f"    Median Query Time: {metrics.get('median_query_time_ms', 'N/A')} ms")
