...
def validate_cache_integrity():
    ...
    """
    setup_logging()
    console.print("[bold cyan]--- Starting Comprehensive Cache Integrity Validation ---[/bold cyan]")
    ...
    try:
        ...
        cache_collection = weaviate_client.collections.get(settings.SEMANTIC_CACHE_INDEX_NAME)
        ...
        weaviate_uuids = set()
        for item in cache_collection.iterator(include_vector=False, return_properties=["cache_id"]):
        ...
        redis_index_uuids = set(str(x) for x in redis_client.zrange("cache_index", 0, -1))
        ...
        console.print(f"Found {len(active_redis_entries)} active data entries in Redis ('cache_entry:*').")
        ...
        is_perfectly_synced = not weaviate_orphans and not redis_index_orphans and not stale_index_entries
        
        if is_perfectly_synced:
            console.print("✅ [bold green]Cache is perfectly in sync. No inconsistencies found.[/bold green]")
            return

        # Report Stale Redis Index Entries (Problem from logs)
        if stale_index_entries:
            table = Table(title="🚨 Stale Redis Index Entries (Causes incorrect stats) 🚨")
            ...
            console.print("✅ [green]Redis 'cache_index' is clean. No stale entries found.[/green]")
        ...
        if weaviate_orphans:
            ...
            console.print("✅ [green]Weaviate is clean. No orphan vector entries found.[/green]")
        ...
        if redis_index_orphans:
            ...
            table.add_column("Orphan UUID in Redis Index", style="magenta", no_wrap=True)
            ...
            console.print("✅ [green]Redis 'cache_index' is consistent with Weaviate.[/green]")

    except Exception as e:
        console.print(f"❌ [bold red]An error occurred during validation: {e}[/bold red]")
    finally:
