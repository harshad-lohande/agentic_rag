...
async def show_cache_stats():
    ...
    print(
        f"🔄 Background GC: {'✅ Running' if stats.get('background_gc_enabled') else '❌ Stopped'}"
    )
    print(
        f"⚡ Async Redis: {'✅ Enabled' if stats.get('async_redis') else '❌ Sync fallback'}"
    )
...
async def benchmark_cache_performance():
    ...
    test_answer = (
        "42, according to Douglas Adams' The Hitchhiker's Guide to the Galaxy."
    )
        ...
        try:
            if hasattr(cache, "redis_client"):
                ...
                if hasattr(cache.redis_client, "zrange"):
                    # Async Redis
                    sample_cache_ids = await cache.redis_client.zrange(
                        index_key, -10, -1
                    )
                else:
                    # Sync Redis
                    sample_cache_ids = await asyncio.to_thread(
                        cache.redis_client.zrange, index_key, -10, -1
                    )
                    ...
                    try:
                        ...
                        if entry:
                            ...
                            sample_entry = {
                                ...
                                if len(entry.get("query", "")) > 100
                                else entry.get("query", ""),
                                "answer": entry.get("answer", "")[:200] + "..."
                                if len(entry.get("answer", "")) > 200
...
def main():
    ...
    parser.add_argument(
        "--file", help="Output file for export action", default="cache_export.json"
    )
