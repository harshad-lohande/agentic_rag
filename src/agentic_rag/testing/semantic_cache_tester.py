...
class SemanticCacheTester:
    ...
    async def create_cache_entry(
        self, query: str, answer: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                if hasattr(self.cache.redis_client, "get"):
                    ...
                    cache_id = await asyncio.to_thread(
                        self.cache.redis_client.get, exact_match_key
                    )
            ...
            return {
                ...
                "cache_id": cache_id.decode()
                if cache_id and hasattr(cache_id, "decode")
                else str(cache_id),
    ...
    async def test_query_similarity(
        self, cached_query: str, test_query: str
        ...
        try:
            ...
            try:
                cross_encoder_similarity = await self.cache._ce_similarity(
                    test_query, cached_query
                )
            except Exception as e:
                logger.debug(f"Cross-encoder similarity test failed: {e}")

            # 2. Lexical similarity (always available)
            lexical_similarity = self.cache._lexical_similarity(
                test_query, cached_query
            )
    ...
    def _predict_cache_hit(
        self, ce_sim: Optional[float], lex_sim: float
    ) -> Tuple[bool, str]:
        """
        ...
        if ce_sim >= ce_min and lex_sim >= lex_min:
            return (
                True,
                f"Rule 2: Cross-encoder & lexical support (ce={ce_sim:.3f}, lex={lex_sim:.2f})",
            )
    ...
    async def test_cache_retrieval(self, query: str) -> CacheTestResponse:
        ...
        try:
            ...
            similarity_score = (
                cached_result.get("similarity") if cached_result else None
            )
    ...
    async def get_cache_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Combine stats
            return {**base_stats, "testing_framework": testing_info}
    ...
    async def bulk_similarity_test(
        self, cached_query: str, test_queries: List[str]
    ) -> List[SimilarityTestResponse]:
        """
    ...
    async def clear_cache_for_testing(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": success,
                "message": "Cache cleared for testing"
                if success
