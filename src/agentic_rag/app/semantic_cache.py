...
class SemanticCache:
    ...
    def __init__(self):
        ...
        self._shutdown_event = (
            asyncio.Event() if asyncio.iscoroutinefunction(lambda: None) else None
        )
    ...
    async def _initialize_clients(self):
        ...
        with self._init_lock:
            ...
            try:
                ...
                logger.info(
                    f"Connected to Redis for semantic caching (async: {AIOREDIS_AVAILABLE})"
                )
                ...
                logger.info(
                    "Semantic cache initialized successfully with optimizations"
                )
                return True
    ...
    async def _cross_encoder_similarity_search(self, query: str, k: int = 1):
        ...
        try:
            ...
            if not cache_ids:
                logger.debug("No cache entries found for cross-encoder search")
                return []
            ...
            for cache_id in cache_ids:
                try:
                    ...
                    if (
                        ce_similarity > 0
                    ):  # Only include if we got a valid similarity score
                        ...
                        mock_doc = type(
                            ...
                            (),
                            {
                                ...
                                "metadata": {"cache_id": cache_id, "doc_id": doc_id},
                            },
                        )()
                        candidates.append((mock_doc, ce_similarity))
                        logger.debug(
                            f"Cross-encoder: query='{query[:30]}...' cached='{cached_query[:30]}...' similarity={ce_similarity:.3f}"
                        )
            ...
            logger.debug(
                f"Cross-encoder search for '{query[:50]}...': found {len(result)} candidates from {len(cache_ids)} total entries"
            )
            return result
    ...
    def _init_weaviate_and_embeddings(self):
        ...
        if self.embedding_model is None:
            ...
            logger.warning(
                "Model registry not initialized for semantic cache, loading embedding model on-demand"
            )
            ...
            logger.debug(
                "Semantic cache using pre-loaded embedding model from registry"
            )
    ...
    async def _is_cache_enabled(self) -> bool:
        ...
        return (
            ...
            and self.cache_vector_store is not None
        )
        ...
        try:
            ...
            if AIOREDIS_AVAILABLE:
                ...
                cache_id = await asyncio.to_thread(
                    self.redis_client.get, exact_match_key
                )
            ...
            if not similar_docs:
                logger.debug(
                    f"No similar documents found in cache for query: {query[:50]}..."
                )
                return None
            ...
            if similarity_score is None:
                logger.warning(
                    "Similarity search returned a document but no score. Cannot validate cache hit."
                )
                return None
            ...
            # Rule 2: Good cross-encoder with lexical support
                ...
                accept = True
                reason = (
                    f"cross-encoder & lexical support (ce={ce_sim:.3f}, lex={lex:.2f})"
                )
            ...
            if not cache_id:
                logger.warning(
                    "Found similar document but it is missing a cache_id in its metadata."
                )
                return None
            ...
            if not cached_result:
                logger.warning(
                    f"Cache entry {cache_id} found in vector index but not in Redis. Cleaning up orphan."
                )
            ...
            logger.info(
                f"Semantic cache hit ({reason}) for query: {query[:50]}... (ce_sim: {ce_sim:.3f})"
            )
                ...
                if ce_sim >= settings.SEMANTIC_CE_SIM_HIGH:  # Very high cross-encoder similarity
                    ...
                    if AIOREDIS_AVAILABLE:
                        ...
                        await asyncio.to_thread(
                            self.redis_client.setex, exact_key, ttl, cache_id
                        )
                        logger.debug(
                            f"Created exact match alias for high-confidence hit (ce_sim={ce_sim:.3f})"
                        )
            except Exception as e:
    ...
    async def _update_cache_access(
        self, cache_id: str, cache_entry: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if AIOREDIS_AVAILABLE:
                await self.redis_client.setex(
                    cache_key, settings.SEMANTIC_CACHE_TTL, json.dumps(cache_entry)
                )
            else:
                await asyncio.to_thread(
    ...
    async def store_answer(
        self, query: str, answer: str, metadata: Dict[str, Any] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if AIOREDIS_AVAILABLE:
                ...
                existing_cache_id = await asyncio.to_thread(
                    self.redis_client.get, exact_match_key
                )

            if existing_cache_id:
                logger.info(
                    f"Updating existing entry due to exact query match: {query[:50]}..."
                )
                return await self._update_existing_cache_entry(
                    existing_cache_id, query, answer, metadata
                )
            ...
            if similar_entries:
                ...
                logger.info(
                    f"Updating similar cache entry (sim: {similarity:.3f}) for query: {query[:50]}..."
                )
                return await self._update_existing_cache_entry(
                    cache_id, query, answer, metadata
                )
    ...
    async def _find_highly_similar_entries(self, query: str) -> List[Tuple[str, float]]:
        ...
        try:
            ...
            if accept:
                logger.info(
                    f"Found similar entry for merging: query='{query[:30]}...' cached='{doc.page_content[:30]}...' ce_sim={score:.3f}"
                )
                out.append((str(cache_id), float(score)))
            else:
                logger.debug(
                    f"Entry not similar enough for merging: ce_sim={score:.3f}"
                )
    ...
    async def _update_existing_cache_entry(
        self,
        cache_id: str,
        query: str,
        new_answer: str,
        ...
        try:
            ...
            if not existing_entry:
                ...
                logger.info(
                    f"Orphaned vector detected for cache_id={cache_id} during update; re-inserting fresh entry."
                )
                try:
                    await self._cleanup_weaviate_vectors([cache_id])
                except Exception:
                    logger.debug(
                        "Best-effort cleanup of orphaned vector failed for cache_id=%s",
                        cache_id,
                    )
                return await self._create_new_cache_entry(
                    query, new_answer, new_metadata
                )
            ...
            if AIOREDIS_AVAILABLE:
                ...
                await asyncio.to_thread(
                    self.redis_client.setex, exact_key, ttl_seconds, cache_id
                )
    ...
    async def _create_new_cache_entry(
        self, query: str, answer: str, metadata: Dict[str, Any] | None
    ) -> bool:
        """
    ...
    async def _manage_cache_size_atomic(self):
        ...
        try:
            ...
            if evicted_cache_ids:
                ...
                logger.info(
                    f"Evicted {len(evicted_cache_ids)} cache entries to maintain size limit"
                )
        ...
        try:
            ...
            for cache_id in cache_ids:
                try:
                    ...
                    logger.debug(
                        f"Error cleaning exact match mapping for {cache_id}: {e}"
                    )
    ...
    async def _run_garbage_collection(self) -> int:
        ...
        try:
            ...
            if AIOREDIS_AVAILABLE:
                ...
                all_indexed_ids = await asyncio.to_thread(
                    self.redis_client.zrange, index_key, 0, -1
                )
            ...
            if all_indexed_ids:
                ...
                if AIOREDIS_AVAILABLE:
                    ...
                    batch_keys = [
                        self._generate_cache_key(id) for id in all_indexed_ids
                    ]
                    exists_results = await asyncio.to_thread(
                        sync_pipeline_exists, batch_keys
                    )
            ...
            if stale_index_ids:
                logger.info(
                    f"Found {len(stale_index_ids)} stale entries in Redis index, cleaning up..."
                )
                if AIOREDIS_AVAILABLE:
                    await self.redis_client.zrem(index_key, *stale_index_ids)
                else:
                    await asyncio.to_thread(
                        self.redis_client.zrem, index_key, *stale_index_ids
                    )

            # Part 2: Clean up orphaned vectors in Weaviate
            orphaned_vectors = await asyncio.to_thread(
                self._find_orphaned_vectors, active_redis_ids
            )
            ...
            if orphaned_vectors:
                logger.info(
                    f"Found {len(orphaned_vectors)} orphaned vectors, cleaning up..."
                )
            ...
            if cleaned_count > 0:
                logger.info(
                    f"Garbage collection completed. Cleaned {cleaned_count} total items."
                )
            else:
                logger.info(
                    "Garbage collection completed, no stale items or orphans found."
                )
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_access_count = (
                total_access_count / sample_size if sample_size > 0 else 0
            )
    ...
    async def clear_cache(self) -> bool:
        ...
        try:
            ...
            if AIOREDIS_AVAILABLE:
                ...
                all_cache_ids = await asyncio.to_thread(
                    self.redis_client.zrange, index_key, 0, -1
                )
            ...
            # Clear Redis entries
            cache_keys = [
                self._generate_cache_key(cache_id) for cache_id in all_cache_ids
            ]
            ...
            if all_keys_to_delete:
                ...
                if AIOREDIS_AVAILABLE:
                    ...
                    await asyncio.to_thread(
                        self.redis_client.delete, *all_keys_to_delete
                    )
            ...
            for cid in cache_ids:
                try:
                    res = collection.data.delete_many(
                        where=Filter.by_property("cache_id").equal(cid)
                    )
                    ...
                    failed += 1
                    logger.debug(
                        f"Error deleting Weaviate vector for cache_id={cid}: {e}"
                    )

            logger.info(
                f"Cleared Weaviate cache collection. Results: {deleted} successful, {failed} failed."
            )
        except Exception as e:
