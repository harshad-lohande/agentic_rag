...
def create_weaviate_vector_store(
    ...
    try:
        ...
        logger.info(
            f"Created Weaviate vector store for index '{index_name}' with HNSW optimization"
        )
        return vector_store
...
def _configure_hnsw_collection(client: weaviate.Client, collection_name: str):
    ...
    try:
        ...
        if client.collections.exists(collection_name):
            logger.info(
                f"Collection '{collection_name}' already exists, HNSW parameters may already be set"
            )
            return
        ...
        logger.info(
            f"HNSW parameters: efConstruction={settings.HNSW_EF_CONSTRUCTION}, "
        ...
        logger.info(
            f"Successfully created collection '{collection_name}' with HNSW optimization"
        )
...
def get_collection_info(client: weaviate.Client, collection_name: str) -> dict:
    ...
    try:
        ...
        info = {
            "exists": True,
            "name": collection_name,
            "vector_index_type": vector_config.__class__.__name__
            if vector_config
            else "Unknown",
        }
