...
def get_last_human_message_content(
...
def get_last_completed_turn_messages(
    messages: List[Union[BaseMessage, Dict[str, Any]]],
...
def replace_last_assistant_message(
    content: str, additional_kwargs: Optional[Dict[str, Any]] = None
) -> Callable[[List[BaseMessage], List[BaseMessage]], List[BaseMessage]]:
    """
...
def create_replacement_message(
    content: str, additional_kwargs: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    return {"messages": [new_message], "_replace_last_ai": True}
